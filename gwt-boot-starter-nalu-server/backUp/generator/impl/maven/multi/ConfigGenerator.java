...
package com.github.nalukit.gwtbootstarternalu.server.service.generator.impl.maven.multi;
...
public class ConfigGenerator {
  ...
  private ConfigGenerator(Builder builder) {
    super();
  ...
  private void generateConfigTomcat()
      throws GeneratorException {
    if (!new File(projectFolder + ConfigGenerator.DIRECTORY_TOMCAT_CONFIG).mkdirs()) {
      logger.debug(">>" + naluGeneraterParms.getArtefactId() + "<< problems creating directory >>" + projectFolder + ConfigGenerator.DIRECTORY_TOMCAT_CONFIG + "<<");
      throw new GeneratorException(">>" + naluGeneraterParms.getArtefactId() + "<< problems creating directory >>" + projectFolder + ConfigGenerator.DIRECTORY_TOMCAT_CONFIG + "<<");
    }

    Configuration freeMarkerConfiguration = new Configuration();

    freeMarkerConfiguration.setClassForTemplateLoading(ConfigGenerator.class,
                                                       "/templates/xml");
    ...
    try (StringWriter out = new StringWriter()) {
      template.process(templateData,
                       out);
      Files.write(Paths.get(new File(this.projectFolder + ConfigGenerator.DIRECTORY_TOMCAT_CONFIG) + File.separator + ConfigGenerator.CONFIG_FILE_NAME),
                  out.toString()
                     .getBytes());
      out.flush();
  ...
  private void generateConfigJetty()
      throws GeneratorException {
    if (!new File(projectFolder + ConfigGenerator.DIRECTORY_JETTY_CONFIG).mkdirs()) {
      logger.debug(">>" + naluGeneraterParms.getArtefactId() + "<< problems creating directory >>" + projectFolder + ConfigGenerator.DIRECTORY_JETTY_CONFIG + "<<");
      throw new GeneratorException(">>" + naluGeneraterParms.getArtefactId() + "<< problems creating directory >>" + projectFolder + ConfigGenerator.DIRECTORY_JETTY_CONFIG + "<<");
    }
    ...
    try (StringWriter out = new StringWriter()) {
      template.process(templateData,
                       out);
      Files.write(Paths.get(new File(this.projectFolder + ConfigGenerator.DIRECTORY_JETTY_CONFIG) + File.separator + ConfigGenerator.CONFIG_FILE_NAME),
                  out.toString()
                     .getBytes());
      out.flush();
    } catch (IOException | TemplateException e) {
